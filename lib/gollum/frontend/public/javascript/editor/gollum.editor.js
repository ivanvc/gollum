...
(function($) {
  ...
  $.GollumEditor = function( IncomingOptions ) {
    ...
    if ( EditorHas.baseEditorMarkup() ) {
      ...
      if ( EditorHas.editSummaryMarkup() || EditorHas.editAuthorMarkup() ||
          EditorHas.renameMarkup() ) {
        if ( EditorHas.editSummaryMarkup() ) {
          $.GollumEditor.Placeholder.add($('#gollum-editor-edit-summary input'));
        }
        if ( EditorHas.editAuthorEmailMarkup() ) {
          $.GollumEditor.Placeholder.add($('#gollum-editor-author-email-field'));
          if ( typeof(Author) !== "undefined" && Author.Email ) {
            $('#gollum-editor-author-email-field').val(Author.Email);
          }
        }
        if ( EditorHas.editAuthorNameMarkup() ) {
          $.GollumEditor.Placeholder.add($('#gollum-editor-author-name-field'));
          if ( typeof(Author) !== "undefined" && Author.Name ) {
            $('#gollum-editor-author-name-field').val(Author.Name);
          }
        }
        ...
        $('#gollum-editor form[name="gollum-editor"]').submit(function( e ) {
          ...
          if ( EditorHas.renameMarkup() &&
            $('#gollum-editor-rename-field input').val() ===
            $('#gollum-editor-page-title').val() ) {
            $('#gollum-editor-rename-field input').remove();
          }
          debug('submitting');
          $(this).unbind('submit');
          $(this).submit();
  ...
  var EditorHas = {
    ...
    /**
     * EditorHas.renameMarkup
     * True if the editor has a rename page field, false otherwise.
     *
     * @return boolean
     */
    renameMarkup: function() {
      return $('input#gollum-editor-rename').length > 0;
    },


    /**
     *  EditorHas.editAuthorEmailMarkup
     *  True if the editor has author email field (Gollum's commit author),
     *  false otherwise.
     *
     *  @return boolean
     */
    editAuthorEmailMarkup: function() {
      return $('input#gollum-editor-author-email-field').length > 0;
    },


    /**
     *  EditorHas.editAuthorNameMarkup
     *  True if the editor has author name field (Gollum's commit author),
     *  false otherwise.
     *
     *  @return boolean
     */
    editAuthorNameMarkup: function() {
      return $('input#gollum-editor-author-name-field').length > 0;
    },
